...
function injectControlUiConfig(html: string, opts: ControlUiInjectionOpts): string {
  ...
  const script =
    `<script>` +
    `window.__OPENBOT_CONTROL_UI_BASE_PATH__=${JSON.stringify(basePath)};` +
    `window.__OPENBOT_ASSISTANT_NAME__=${JSON.stringify(
      assistantName ?? DEFAULT_ASSISTANT_IDENTITY.name,
    )};` +
    `window.__OPENBOT_ASSISTANT_AVATAR__=${JSON.stringify(
      assistantAvatar ?? DEFAULT_ASSISTANT_IDENTITY.avatar,
    )};` +
    `</script>`;
  // Check if already injected
  if (html.includes("__OPENBOT_ASSISTANT_NAME__")) {
    return html;
  }
