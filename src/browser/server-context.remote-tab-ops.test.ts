...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenBotChrome: vi.fn(async () => {
  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        openbot: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("openbot");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const openbot = ctx.forProfile("openbot");

    const opened = await openbot.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("openbot")?.lastTargetId).toBe("CREATED");
  });
});
