...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-embedded-agent-"));
...
const ensureModels = (cfg: OpenBotConfig) => ensureOpenBotModelsJson(cfg, agentDir) as unknown;
