...
`;
    ...
    expect(parsed.openbot.emoji).toBe("💾");
    expect(parsed.openbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openbot.emoji).toBe("📝");
    expect(parsed.openbot.events).toEqual(["command"]);
    expect(parsed.openbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openbot": {"events": ["test"]}}');
  });
...
describe("resolveOpenBotMetadata", () => {
  it("extracts openbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenBotMetadata(frontmatter);
  ...
  it("returns undefined when openbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenBot" },
            { id: "npm", kind: "npm", package: "@openbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openbot/hook");
  });
...
homepage: https://docs.openbot.ai/hooks#session-memory
metadata:
  {
    "openbot":
      {
...
`;
    ...
    const openbot = resolveOpenBotMetadata(frontmatter);
    expect(openbot).toBeDefined();
    expect(openbot?.emoji).toBe("💾");
    expect(openbot?.events).toEqual(["command:new"]);
    expect(openbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(openbot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openbot = resolveOpenBotMetadata(frontmatter);
    expect(openbot?.emoji).toBe("disk");
    expect(openbot?.events).toEqual(["command:new"]);
  });
});
