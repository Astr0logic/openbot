...
import type { OpenBotConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: OpenBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenBotConfig),
...
export function getCommandDetection(_cfg?: OpenBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenBotConfig) {
