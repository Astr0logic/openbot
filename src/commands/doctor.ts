...
function resolveMode(cfg: OpenBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenBot doctor");

  const root = await resolveOpenBotPackageRoot({
  ...
  let cfg: OpenBotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("openbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openbot doctor --fix")}" to apply changes.`);
  }
