...
import type { OpenBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareOpenBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): OpenBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenBotConfig;
}
...
function stampConfigVersion(cfg: OpenBotConfig): OpenBotConfig {
...
function warnIfConfigFromFuture(cfg: OpenBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenBotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpenBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpenBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenBotConfig) {
    clearConfigCache();
...
// module scope. `OPENBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENBOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export async function writeConfigFile(cfg: OpenBotConfig): Promise<void> {
  clearConfigCache();
