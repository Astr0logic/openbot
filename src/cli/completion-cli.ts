...
export async function installCompletion(shell: string, yes: boolean, binName = "openbot") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c openbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'openbot gateway' and not 'openbot other gateway', we need to check the sequence.
