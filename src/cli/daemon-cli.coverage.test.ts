...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OPENBOT_STATE_DIR: process.env.OPENBOT_STATE_DIR,
    OPENBOT_CONFIG_PATH: process.env.OPENBOT_CONFIG_PATH,
    OPENBOT_GATEWAY_PORT: process.env.OPENBOT_GATEWAY_PORT,
    OPENBOT_PROFILE: process.env.OPENBOT_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENBOT_STATE_DIR = "/tmp/openbot-cli-state";
    process.env.OPENBOT_CONFIG_PATH = "/tmp/openbot-cli-state/openbot.json";
    delete process.env.OPENBOT_GATEWAY_PORT;
    delete process.env.OPENBOT_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENBOT_STATE_DIR !== undefined) {
      process.env.OPENBOT_STATE_DIR = originalEnv.OPENBOT_STATE_DIR;
    } else {
      delete process.env.OPENBOT_STATE_DIR;
    }

    if (originalEnv.OPENBOT_CONFIG_PATH !== undefined) {
      process.env.OPENBOT_CONFIG_PATH = originalEnv.OPENBOT_CONFIG_PATH;
    } else {
      delete process.env.OPENBOT_CONFIG_PATH;
    }

    if (originalEnv.OPENBOT_GATEWAY_PORT !== undefined) {
      process.env.OPENBOT_GATEWAY_PORT = originalEnv.OPENBOT_GATEWAY_PORT;
    } else {
      delete process.env.OPENBOT_GATEWAY_PORT;
    }

    if (originalEnv.OPENBOT_PROFILE !== undefined) {
      process.env.OPENBOT_PROFILE = originalEnv.OPENBOT_PROFILE;
    } else {
      delete process.env.OPENBOT_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENBOT_PROFILE: "dev",
        OPENBOT_STATE_DIR: "/tmp/openbot-daemon-state",
        OPENBOT_CONFIG_PATH: "/tmp/openbot-daemon-state/openbot.json",
        OPENBOT_GATEWAY_PORT: "19001",
      },
