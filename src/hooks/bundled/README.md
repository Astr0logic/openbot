...
This directory contains hooks that ship with OpenBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.openbot/workspace`)

**Enable**:

```bash
openbot hooks enable session-memory
```
...
**Output**: `~/.openbot/logs/commands.log`

**Enable**:

```bash
openbot hooks enable command-logger
```
...
**Docs**: https://docs.openbot.ai/hooks/soul-evil

**Enable**:

```bash
openbot hooks enable soul-evil
```
...
homepage: https://docs.openbot.ai/hooks#my-hook
metadata:
  { "openbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.openbot/hooks/` (shared across workspaces)
...
```bash
openbot hooks info session-memory
```
...
```bash
openbot hooks enable session-memory
openbot hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.openbot/openbot.json`:
...
2. Restart gateway: `pkill -9 -f 'openbot.*gateway' && pnpm openbot gateway`
3. Enable the hook: `openbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openbot.ai/hooks
