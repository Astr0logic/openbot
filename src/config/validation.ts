...
import type { OpenBotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpenBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpenBotConfig)),
    ),
  };
}
