...
beforeAll(() => {
  previousProfile = process.env.OPENBOT_PROFILE;
  process.env.OPENBOT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENBOT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openbot-root.js", () => ({
  resolveOpenBotPackageRoot: vi.fn().mockResolvedValue("/tmp/openbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openbot/pnpm-lock.yaml",
      markerPath: "/tmp/openbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpenBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openbot status --all") ||
          l.includes("openbot --profile isolated status --all") ||
          l.includes("openbot status --all") ||
          l.includes("openbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENBOT_GATEWAY_TOKEN = "abcd1234";
    try {
