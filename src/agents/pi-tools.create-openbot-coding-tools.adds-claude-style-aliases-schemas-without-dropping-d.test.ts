...
const defaultTools = createOpenBotCodingTools();

describe("createOpenBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createOpenBotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from openbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "openbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "openbot-workspace"),
    ...
    const tools = createOpenBotCodingTools({ sandbox });
