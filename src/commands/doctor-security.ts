...
import type { OpenBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openbot doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "openbot config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("openbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
