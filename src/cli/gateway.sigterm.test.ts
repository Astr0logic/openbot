...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENBOT_NO_RESPAWN: "1",
      OPENBOT_STATE_DIR: stateDir,
      OPENBOT_SKIP_CHANNELS: "1",
      OPENBOT_SKIP_GMAIL_WATCHER: "1",
      OPENBOT_SKIP_CRON: "1",
      OPENBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openbot-entry-bootstrap.mjs");
