...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-openbot-message-channel") ?? "");
  const accountId = getHeader(req, "x-openbot-account-id")?.trim() || undefined;
  ...
  const allTools = createOpenBotTools({
