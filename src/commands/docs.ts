...
const SEARCH_TOOL = "https://docs.openbot.ai/mcp.SearchOpenBot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openbot.ai/");
      runtime.log(`Search: ${formatCliCommand('openbot docs "your query"')}`);
    }
    return;
  }
