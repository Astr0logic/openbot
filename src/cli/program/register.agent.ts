...
${formatHelpExamples([
  ['openbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openbot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openbot agents set-identity --agent main --name "OpenBot" --emoji "🦞"', "Set name + emoji."],
  ["openbot agents set-identity --agent main --avatar avatars/openbot.png", "Set avatar path."],
  [
    "openbot agents set-identity --workspace ~/.openbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openbot agents set-identity --identity-file ~/.openbot/workspace/IDENTITY.md --agent main",
