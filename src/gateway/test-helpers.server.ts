...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENBOT_STATE_DIR;
  previousConfigPath = process.env.OPENBOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENBOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENBOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENBOT_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENBOT_STATE_DIR = path.join(tempHome, ".openbot");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "openbot-test-"))
    : path.join(tempHome, ".openbot-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPENBOT_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENBOT_CONFIG_PATH;
    } else {
      process.env.OPENBOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENBOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENBOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENBOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENBOT_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENBOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.OPENBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENBOT_GATEWAY_TOKEN = fallbackToken;
  }
