...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openbot-profile-decorated");
}
 ...
 */
export function decorateOpenBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENBOT_BROWSER_COLOR).toUpperCase();
