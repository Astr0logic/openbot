...
function candidateBinDirs(opts: EnsureOpenBotPathOpts): string[] {
  ...
  // Bundled macOS app: `openbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openbot");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/openbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenBotCliOnPath(opts: EnsureOpenBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENBOT_PATH_BOOTSTRAPPED = "1";
