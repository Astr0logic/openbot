...
import { createOpenBotCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openbot-ws-", async (workspaceDir) => {
      await withTempDir("openbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenBotCodingTools({ workspaceDir });
    ...
    await withTempDir("openbot-ws-", async (workspaceDir) => {
      await withTempDir("openbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello openbot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openbot-ws-", async (workspaceDir) => {
      await withTempDir("openbot-override-", async (overrideDir) => {
        const tools = createOpenBotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openbot-sandbox-", async (sandboxDir) => {
      await withTempDir("openbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "openbot-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "openbot-sandbox:bookworm-slim",
            containerPrefix: "openbot-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createOpenBotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
