...
import { resolveOpenBotPackageRoot } from "../infra/openbot-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "openbot doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "Fresh code, same jellyfish. Miss me?",
  "Back and better. Did you even notice I was gone?",
  "Update complete. I learned some new tricks while I was out.",
  "Upgraded! Now with 23% more sass.",
  "I've evolved. Try to keep up.",
  "New version, who dis? Oh right, still me but shinier.",
  "Patched, polished, and ready to sting. Let's go.",
  "The jellyfish has bloomed. Longer tentacles, sharper stings.",
...
const DEFAULT_PACKAGE_NAME = "openbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENBOT_REPO_URL = "https://github.com/openbot/openbot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".openbot");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openbot@")) {
    return trimmed.slice("openbot@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.OPENBOT_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENBOT_GIT_DIR to an empty folder or an openbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENBOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveOpenBotPackageRoot({
  ...
  defaultRuntime.log(theme.heading("OpenBot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OpenBot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenBot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openbot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g openbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g openbot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPENBOT_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPENBOT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("openbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("openbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `openbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENBOT_GIT_DIR to an empty folder or an openbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OPENBOT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openbot update", "Update a source checkout (git)"],
        ["openbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["openbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["openbot update --tag beta", "One-off update to a dist-tag or version"],
        ["openbot update --no-restart", "Update without restarting the service"],
        ["openbot update --json", "Output result as JSON"],
        ["openbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openbot update wizard", "Interactive update wizard"],
        ["openbot --update", "Shorthand for openbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openbot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openbot update status", "Show channel + version status."],
          ["openbot update status --json", "JSON output."],
          ["openbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openbot.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
