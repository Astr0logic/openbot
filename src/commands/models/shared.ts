...
export async function updateConfig(
  mutator: (cfg: OpenBotConfig) => OpenBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
