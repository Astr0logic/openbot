...
const prevBundledDir = process.env.OPENBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpenBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPENBOT_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadOpenBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openbot/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        openbot: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
