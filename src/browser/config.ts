import type { BrowserConfig, BrowserProfileConfig, OpenBotConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENBOT_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenBot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openbot/openbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenBotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openbot";
