...
beforeEach(() => {
  ...
  resolveOpenBotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENBOT_STATE_DIR;
  originalUpdateInProgress = process.env.OPENBOT_UPDATE_IN_PROGRESS;
  process.env.OPENBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpenBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/openbot-root.js", () => ({
  resolveOpenBotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "openbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openbot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
