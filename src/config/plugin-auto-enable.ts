import type { OpenBotConfig } from "./config.js";
...
function isTelegramConfigured(cfg: OpenBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OpenBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OpenBotConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OpenBotConfig): boolean {
...
function isIMessageConfigured(cfg: OpenBotConfig): boolean {
...
function isWhatsAppConfigured(cfg: OpenBotConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpenBotConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OpenBotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpenBotConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: OpenBotConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OpenBotConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: OpenBotConfig, pluginId: string): OpenBotConfig {
...
function enablePluginEntry(cfg: OpenBotConfig, pluginId: string): OpenBotConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
