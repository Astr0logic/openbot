...
export function listAgentEntries(cfg: OpenBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenBotConfig): AgentSummary[] {
