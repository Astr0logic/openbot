...
const execHostEnforced = process.env.OPENBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPENBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpenBotCliOnPath({ pathEnv: process.env.PATH ?? "" });
