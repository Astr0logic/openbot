...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENBOT_SKIP_GMAIL_WATCHER;
    process.env.OPENBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
