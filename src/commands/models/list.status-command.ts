...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openbot models auth setup-token")}\` or \`${formatCliCommand("openbot configure")}\`.`
          : `Run \`${formatCliCommand("openbot configure")}\` or set an API key env var.`;
