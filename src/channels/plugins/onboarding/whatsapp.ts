...
function setWhatsAppDmPolicy(cfg: OpenBotConfig, dmPolicy: DmPolicy): OpenBotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpenBotConfig, allowFrom?: string[]): OpenBotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpenBotConfig, selfChatMode: boolean): OpenBotConfig {
...
async function detectWhatsAppLinked(cfg: OpenBotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so OpenBot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OpenBot" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("openbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
