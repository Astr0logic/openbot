...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPENBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENBOT_STATE_DIR = path.join(home, ".openbot");
    ...
    await fs.mkdir(path.join(home, ".openbot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.openbot${path.sep}media`);
