...
import { OpenBotSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof OpenBotSchema.toJSONSchema>;
...
const FIELD_LABELS: Record<string, string> = {
  ...
  "gateway.notary.mode": "Data Notary Mode",
  "gateway.notary.failOpen": "Notary Fail Open",
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when OpenBot writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /openbot).",
  ...
  "gateway.notary.mode":
    'Traffic observation mode ("off" = disabled, "observe" = log receipts but do not block, "enforce" = block invalid traffic).',
  "gateway.notary.failOpen":
    "If true, traffic is allowed even if the Notary service fails or times out (default: true).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $OPENBOT_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.openbot/memory/{agentId}.sqlite).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `openbot plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.openbot/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/openbot",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/openbot.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = OpenBotSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "OpenBotConfig";
