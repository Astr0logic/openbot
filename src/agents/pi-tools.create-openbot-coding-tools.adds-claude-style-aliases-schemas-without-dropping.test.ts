...
import { __testing, createOpenBotCodingTools } from "./pi-tools.js";
...
describe("createOpenBotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenBotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openbot-outside.txt");
