...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENBOT_CONTROL_UI_BASE_PATH__ = "/openbot";
    const app = mountApp("/openbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openbot/sessions");
  });
