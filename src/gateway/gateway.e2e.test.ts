...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.OPENBOT_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-gw-mock-home-"));
      ...
      process.env.OPENBOT_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.OPENBOT_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "openbot");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.openbot-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".openbot");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "openbot.json");
      ...
      process.env.OPENBOT_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.OPENBOT_CONFIG_PATH = prev.configPath;
        process.env.OPENBOT_GATEWAY_TOKEN = prev.token;
        process.env.OPENBOT_SKIP_CHANNELS = prev.skipChannels;
        process.env.OPENBOT_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.OPENBOT_SKIP_CRON = prev.skipCron;
        process.env.OPENBOT_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.OPENBOT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.OPENBOT_STATE_DIR = prev.stateDir;
      process.env.OPENBOT_CONFIG_PATH = prev.configPath;
      process.env.OPENBOT_GATEWAY_TOKEN = prev.token;
      process.env.OPENBOT_SKIP_CHANNELS = prev.skipChannels;
      process.env.OPENBOT_SKIP_GMAIL_WATCHER = prev.skipGmail;
