...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenBotAgentDir: vi.fn().mockReturnValue("/tmp/openbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenBotAgentDir: mocks.resolveOpenBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/openbot-agent-custom");
