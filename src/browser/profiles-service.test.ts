...
vi.mock("./chrome.js", () => ({
  resolveOpenBotUserDataDir: vi.fn(() => "/tmp/openbot-test/openbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOpenBotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "openbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveOpenBotUserDataDir).mockReturnValue(userDataDir);
