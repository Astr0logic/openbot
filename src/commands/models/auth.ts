...
import { readConfigFileSnapshot, type OpenBotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpenBotConfig, model: string): OpenBotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("openbot plugins install")}\`.`,
    );
  }
