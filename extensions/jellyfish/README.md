...
Adds the `jellyfish` agent tool as an **optional** plugin tool.

## What this is

- Jellyfish is a standalone workflow shell (typed JSON-first pipelines + approvals/resume).
- This plugin integrates Jellyfish with OpenClaw _without core changes_.
    ...
    "list": [
      {
        ...
        "tools": {
          "allow": [
            "jellyfish" // plugin id (enables all tools from this plugin)
          ]
        }
      }
    ]
...
## Using `openclaw.invoke` (Jellyfish → OpenClaw tools)

Some Jellyfish pipelines may include a `openclaw.invoke` step to call back into OpenClaw tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).
    ...
    "list": [
      {
        "id": "main",
        "tools": {
          "allow": ["jellyfish", "web_fetch", "web_search", "gog", "gh"],
          "deny": ["gateway"],
        },
      },
    ],
...
- Runs the `jellyfish` executable as a local subprocess.
- Does not manage OAuth/tokens.
- Uses timeouts, stdout caps, and strict JSON envelope parsing.
- Prefer an absolute `jellyfishPath` in production to avoid PATH hijack.
