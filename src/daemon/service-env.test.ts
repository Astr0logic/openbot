...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENBOT_GATEWAY_PORT).toBe("18789");
    expect(env.OPENBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENBOT_SERVICE_MARKER).toBe("openbot");
    expect(env.OPENBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENBOT_SERVICE_VERSION).toBe("string");
    expect(env.OPENBOT_SYSTEMD_UNIT).toBe("openbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENBOT_LAUNCHD_LABEL).toBe("ai.openbot.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENBOT_SYSTEMD_UNIT).toBe("openbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENBOT_LAUNCHD_LABEL).toBe("ai.openbot.work");
    }
  });
});
