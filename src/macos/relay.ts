...
declare const __OPENBOT_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENBOT_VERSION__ === "string" && __OPENBOT_VERSION__) ||
  process.env.OPENBOT_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureOpenBotCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[openbot] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[openbot] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
