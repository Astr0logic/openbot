...
/**
 * Cleanup suspended OpenBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-cli-images-"));
  const paths: string[] = [];
