...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENBOT_GATEWAY_PASSWORD");
  });
