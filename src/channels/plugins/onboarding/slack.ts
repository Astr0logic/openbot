...
function setSlackDmPolicy(cfg: OpenBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenBot`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/openbot",
          description: "Send a message to OpenBot",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: OpenBotConfig, allowFrom: string[]): OpenBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
