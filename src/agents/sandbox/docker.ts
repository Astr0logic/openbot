...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openbot.sandbox=1");
  args.push("--label", `openbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `openbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("openbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openbot sandbox recreate --all");
}
