...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenBot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.openbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openbot gateway probe")}`);
  }
}
