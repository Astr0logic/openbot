...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openbot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".openbot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenBotConfig> = {},
): OpenBotConfig {
  const base: OpenBotConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "openbot"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".openbot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
