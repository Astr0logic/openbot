...
export function triggerOpenBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENBOT_SYSTEMD_UNIT,
        process.env.OPENBOT_PROFILE,
      );
  ...
  const label =
    process.env.OPENBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENBOT_PROFILE);
