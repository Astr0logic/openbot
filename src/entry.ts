...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OPENBOT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OPENBOT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OPENBOT_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[openbot] ${parsed.error}`);
    process.exit(2);
  }
