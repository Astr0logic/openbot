...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "openbot doctor",
        managerScriptArgs(manager, "openbot", ["doctor", "--non-interactive"]),
        gitRoot,
        { OPENBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
