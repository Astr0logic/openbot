...
function resolveMainSessionKeyForSandbox(params: { cfg?: OpenBotConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: OpenBotConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`openbot sandbox explain --session ${runtime.sessionKey}`)}`,
  );
