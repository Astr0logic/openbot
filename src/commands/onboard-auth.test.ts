...
const requireAgentDir = () => {
  const agentDir = process.env.OPENBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENBOT_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPENBOT_STATE_DIR;
  const previousAgentDir = process.env.OPENBOT_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      ...
      process.env.OPENBOT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPENBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-oauth-"));
    process.env.OPENBOT_STATE_DIR = tempStateDir;
    process.env.OPENBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openbot-minimax-"));
    process.env.OPENBOT_STATE_DIR = tempStateDir;
    process.env.OPENBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENBOT_AGENT_DIR;
