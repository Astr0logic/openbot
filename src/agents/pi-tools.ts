...
function resolveExecConfig(cfg: OpenBotConfig | undefined) {
...
export function createOpenBotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createOpenBotReadTool(freshReadTool)];
    }
