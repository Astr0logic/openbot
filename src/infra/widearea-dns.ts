...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openbot-gw");
  ...
  records.push(`_openbot-gw._tcp IN PTR ${instanceLabel}._openbot-gw._tcp`);
  records.push(`${instanceLabel}._openbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openbot-content-hash: ${contentHash}\n${contentBody}`;
}
