...
export function resolveHooksConfig(cfg: OpenBotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-openbot-token"] === "string" ? req.headers["x-openbot-token"].trim() : "";
  if (headerToken) {
